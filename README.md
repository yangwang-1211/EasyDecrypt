...
该工具基于 Python + Mitmproxy 构建一个上下游代理架构，主要用于解决安全测试中流量加密不可见、验签无法绕过的难题。工具亮点：
...
下游代理：负责接收 Burp 修改后的明文请求，自动完成重新加密与签名生成，确保数据能够以符合业务协议的形式正确发送到目标服务器；介绍来自服务器的响应报文，对加密的响应报文进行自动解密和结构还原，使 Burp 等抓包工具能够直接查看和修改明文数据
